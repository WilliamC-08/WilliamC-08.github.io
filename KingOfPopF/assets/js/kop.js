...
function songlyrics(song_id){
  ...
  .then(function(data){
    ...
    data.lirik.forEach(function(song){
     ...
     `;
     songtitle=`<h4 class="center-align" style="margin-bottom: 30px;">${song.song_title}</h4>`;
   });
...
function songlist(){
  ...
  .then(function(data){
    var testHTML="";
    data.list.forEach(function(songs){
      testHTML +=`
      <div class="collection">
        <a href="lyrics.html?id=${songs.song_id}" class="collection-item">${songs.song_title}</a>
      </div>
      `;
      document.getElementById("songlist").innerHTML = testHTML;
    });
  })
  .catch(error);
}
...
function songsearch(title){
  fetch(endpoint_url+"songs/songsearch"+"?songtitle="+title)
  .then(status)
  .then(json)
  .then(function(data){
    var titleHTML="";
   ...
   document.getElementById("searchlist").innerHTML=titleHTML;
  })
  .catch(error);
}
